...
import { Portfolio, PortfolioSchema } from "./schema";
...
export const PortfolioForm = ({ portfolio }: PortfolioFormProps) => {
    ...
    //     startTransition(() => {
    //         createPortfolio(JSON.parse(JSON.stringify(data))).then((data) => {
    //             if (data?.success) {
    //                 alert(data?.success);
    //             }
    ...
    return (
        <>
            ...
            {/* {active === 2 && <EducationManager data={portfolio} setData={setData} setActive={setActive} />}
            {active === 3 && <ProjectsManager data={portfolio} setData={setData} setActive={setActive} />} */}
